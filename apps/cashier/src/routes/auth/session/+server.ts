...
/** POST /auth/session — exchange credentials for an httpOnly cashier session. */
export const POST: RequestHandler = async ({ request, cookies }) => {
  let payload: { email?: string; password?: string; store_id?: string };
  try {
  ...
  if (!storeId) throw error(400, "Store ID is required.");
  ...
  const resolvedStore =
    data.claims?.store_id ?? data.claims?.storeId ?? data.store_id ?? storeId;
  if (role !== REQUIRED_ROLE) throw error(403, "Cashier access only.");
  if (!token) throw error(502, "Invalid login response from gateway.");

  writeSession(cookies, { token, role, storeId: resolvedStore });
  return json({ role, storeId: resolvedStore });
};
